...
export default function Sponsor2() {
  useEffect(() => {
    ...
    const sponsorSocialLinks: Record<
      string,
      { facebook: string; website: string; instagram: string }
    > = {
      MEZORN: {
    ...
    const root = document.getElementById('sponsor2-root');
    if (!root) return;

    const hoverCards = root.querySelectorAll('.logo-marquee .hover-card');
    hoverCards.forEach((card) => {
      if (card.querySelector('.socials')) return;

      const labelNode = card.querySelector('.side-label');
      const sponsorKey = labelNode ? labelNode.textContent?.trim().toUpperCase() ?? '' : '';
      const links = sponsorSocialLinks[sponsorKey] || defaultSocialLinks;
    ...
    const rows = root.querySelectorAll('.pattern-row');
        ...
        const animateGuides = (now: number) => {
          ...
          guides.forEach(
            ({ path, dot, highlight, length, isNaturalLeftToRight, isOuterStroke, startOffset }) => {
              ...
              (dot as SVGCircleElement).style.opacity = t > 0 ? '1' : '0';
              highlight.style.strokeDashoffset = currentOffset.toFixed(2);
            }
          );

          if (t < 1) {
            const raf = requestAnimationFrame(animateGuides);
            animationFrameIds.push(raf);
          }
        };

        const raf2 = requestAnimationFrame(animateGuides);
        animationFrameIds.push(raf2);
      });

      animationFrameIds.push(raf1);
    });

    return () => {
      animationFrameIds.forEach((id) => cancelAnimationFrame(id));
      root.querySelectorAll('.deer-highlight, .guide-dot, .socials').forEach((el) => el.remove());
    };
  }, []);
    ...
    <>
      <style>{`
        * {
        ...
        #sponsor2-root .sponsors-section {
            width: 100%;
            overflow: hidden;
            background: #21c7f4;
            padding: 48px 0 56px;
        ...
        #sponsor2-root .pattern-segment {
            flex: 0 0 auto;
            height: clamp(34px, 4.5vw, 64px);
            width: 100vw;
        }

        #sponsor2-root .pattern-svg {
            width: 100%;
            height: 100%;
            display: block;
            overflow: visible;
        ...
        #sponsor2-root .pattern-svg .deer-stroke {
            fill: none;
            stroke: white;
            stroke-width: 2;
        ...
        #sponsor2-root .pattern-svg .deer-highlight {
            fill: none;
            stroke: #ffffff;
            stroke-width: 2.2;
        ...
        #sponsor2-root .pattern-svg .guide-dot {
            fill: #ffffff;
            opacity: 0;
        ...
        #sponsor2-root .hover-layer {
            position: absolute;
            inset: 10px 0 14px;
            z-index: 4;
        }

        #sponsor2-root .hover-track {
            display: flex;
            align-items: center;
            gap: var(--slot-gap);
            width: max-content;
        ...
        #sponsor2-root .logo-marquee:hover .logo-track {
            animation-play-state: paused;
        }

        #sponsor2-root .logo-marquee:hover .hover-track {
            animation-play-state: paused;
        }

        #sponsor2-root .logo-item {
            --card-bg: #FBAE18;
            position: relative;
            flex: 0 0 auto;
            width: var(--card-w);
        ...
        #sponsor2-root .hover-card:has(.side-label.side-right) {
            --ger-x: left 0%;
        }

        #sponsor2-root .logo-marquee:hover .logo-face {
            opacity: 0;
            transform: scale(.88);
        }

        #sponsor2-root .logo-marquee:hover .hover-card {
            opacity: 1;
            transform: translateY(0) scale(1);
            pointer-events: auto;
        }

        #sponsor2-root .hover-card .side-label {
            position: absolute;
        ...
        #sponsor2-root .hover-card .side-label.side-right {
            left: auto;
        ...
        #sponsor2-root .hover-card .card-title {
            position: absolute;
            width: 80%;
            left: 50%;
            top: 9%;
        ...
        #sponsor2-root .hover-card .card-logo {
            position: absolute;
            width: 73.33%;
            height: 33.91%;
            left: 50%;
        ...
        #sponsor2-root .hover-card .card-logo img {
            max-width: 100%;
            max-height: 100%;
            object-fit: contain;
            display: block;
        ...
        #sponsor2-root .hover-card .socials {
            position: absolute;
            right: 5%;
            bottom: 7%;
            display: flex;
        ...
        #sponsor2-root .hover-card .socials a {
        ...
        #sponsor2-root .hover-card .socials svg {
            width: 100%;
            height: 100%;
            display: block;
        }

        #sponsor2-root .hover-card .socials img {
            width: 100%;
            height: 100%;
            display: block;
            object-fit: contain;
        ...
        @media (max-width: 1024px) {
            ...
            #sponsor2-root .hover-track .hover-slot:nth-child(n+6) {
                display: none;
            }
          ...
          <div className="container">
            ...
            <div className="logo-marquee">
              <div className="logo-track">
                <div className="logo-item small">
                  <div className="logo-face"><img src="MezornLogo.png" alt="Mezorn" /></div>
                </div>

                <div className="logo-item large">
                  <div className="logo-face"><img src="ZangiaLogo.png" alt="Zangia" /></div>
                </div>

                <div className="logo-item small">
                  <div className="logo-face"><img src="NashaTechLogo.png" alt="Nasha Tech" /></div>
                </div>

                <div className="logo-item large">
                  <div className="logo-face"><img src="ItParkLogo.png" alt="IT Park" /></div>
                </div>
              ...
              <div className="hover-layer">
                <div className="hover-track">
                  <div className="hover-slot" style={{ ['--card-bg' as string]: '#FBAE18' }}>
                    <div className="hover-card">
                      <div className="bg-shape"></div>
                      <div className="side-label">MEZORN</div>
                      <div className="card-title">ЕРӨНХИЙ ИВЭЭН ТЭТГЭГЧ</div>
                      <div className="card-logo"><img src="MezornLogo.png" alt="Mezorn" /></div>
                    </div>
                  </div>

                  <div className="hover-slot" style={{ ['--card-bg' as string]: '#EF4323' }}>
                    <div className="hover-card">
                      <div className="bg-shape"></div>
                      <div className="side-label side-right">ZANGIA</div>
                      <div className="card-title">ИВЭЭН ТЭТГЭГЧ</div>
                      <div className="card-logo"><img src="ZangiaLogo.png" alt="Zangia" /></div>
                    </div>
                  </div>
                  ...
                  <div className="hover-slot" style={{ ['--card-bg' as string]: '#EF4323' }}>
                    <div className="hover-card">
                      <div className="bg-shape"></div>
                      <div className="side-label side-right">NASHA</div>
                      <div className="card-title">ИВЭЭН ТЭТГЭГЧ</div>
                      <div className="card-logo"><img src="NashaTechLogo.png" alt="Nasha Tech" /></div>
                    </div>
                  </div>

                  <div className="hover-slot" style={{ ['--card-bg' as string]: '#5B3F9B' }}>
                    <div className="hover-card">
                      <div className="bg-shape"></div>
                      <div className="side-label">ITPARK</div>
                      <div className="card-title">ХАМТРАН АЖИЛЛАГЧ</div>
                      <div className="card-logo"><img src="ItParkLogo.png" alt="IT Park" /></div>
                    </div>
                  </div>

                  <div className="hover-slot" style={{ ['--card-bg' as string]: '#057242' }}>
                    <div className="hover-card">
                      <div className="bg-shape"></div>
                      <div className="side-label side-right">SYSCO</div>
                      <div className="card-title">ЗОХИОН БАЙГУУЛАГЧ</div>
                    </div>
                  </div>
            ...
            <div className="pattern-row pattern-bottom bottom">
              <div className="pattern-track">
                <div className="pattern-segment" aria-hidden="true">
                  <svg className="pattern-svg" viewBox="0 0 1440 64" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
